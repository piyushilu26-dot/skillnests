...
function tryCalculate(expression: string): number | null {
  const cleaned = expression.replace(/[×x]/gi, "*").replace(/[÷]/g, "/").replace(/[−–—]/g, "-").replace(/\s+/g, "").replace(/\?+$/, "");
  if (!/^[0-9+\-*/%.()^]+$/.test(cleaned) || !/[+\-*/%^]/.test(cleaned)) return null;
  let i = 0;
  const parseExpression = (): number => {
    let value = parseTerm();
    while (cleaned[i] === "+" || cleaned[i] === "-") { const op = cleaned[i++]; const rhs = parseTerm(); value = op === "+" ? value + rhs : value - rhs; }
    return value;
  };
  const parseTerm = (): number => {
    let value = parsePower();
    while (cleaned[i] === "*" || cleaned[i] === "/" || cleaned[i] === "%") { const op = cleaned[i++]; const rhs = parsePower(); if (op === "*") value *= rhs; else if (op === "/") { if (rhs === 0) throw new Error("division by zero"); value /= rhs; } else value %= rhs; }
    return value;
  };
  const parsePower = (): number => { let value = parseUnary(); if (cleaned[i] === "^") { i++; value = Math.pow(value, parsePower()); } return value; };
  const parseUnary = (): number => { if (cleaned[i] === "+") { i++; return parseUnary(); } if (cleaned[i] === "-") { i++; return -parseUnary(); } return parsePrimary(); };
  const parsePrimary = (): number => {
    if (cleaned[i] === "(") { i++; const value = parseExpression(); if (cleaned[i] !== ")") throw new Error("missing parenthesis"); i++; return value; }
    const start = i; while (/[0-9.]/.test(cleaned[i] || "")) i++; if (start === i) throw new Error("expected number");
    const value = Number(cleaned.slice(start, i)); if (!Number.isFinite(value)) throw new Error("invalid number"); return value;
  };
  try { const result = parseExpression(); return i === cleaned.length && Number.isFinite(result) ? result : null; } catch { return null; }
}

function formatNumber(value: number) { return Number.isInteger(value) ? String(value) : String(Number(value.toFixed(10))); }
...
function localFallback(prompt: string, seconds: number, history: Message[]) {
  ...
  if (/newton.*second law|second law.*newton|force.*mass.*acceleration/.test(p)) return "Newton's second law states that the net force on an object equals its mass multiplied by its acceleration: F = ma. In simple terms, a larger force produces greater acceleration, while a larger mass needs more force for the same acceleration.";
  if (/example|real[- ]life example|give me an example/.test(p)) {
    const previous = [...history].reverse().find((m) => m.role === "user" && m.content);
    if (previous && /newton|force|acceleration|second law/.test(previous.content.toLowerCase())) return "A simple example is pushing a shopping cart: pushing it harder gives it greater acceleration, while a heavier cart needs more force to get the same acceleration. That is F = ma in everyday life.";
  ...
  if (/explain.*easier|make.*easier|simpler|simple terms/.test(p)) {
    ...
    if (previous && /newton|force|acceleration|f = ma/.test(previous.content.toLowerCase())) return "Think of it like this: push a light cart and it speeds up easily; push a heavy cart with the same force and it speeds up less. More force means more acceleration, and more mass means less acceleration for the same force.";
  }
  ...
  if (/schedule|study plan|manage my time/.test(p)) return ["Here’s a starter study schedule (assuming you’re free from 5:00 PM–10:00 PM):","","5:00–5:15 PM — Plan the session + review goals","5:15–6:15 PM — Physics: learn/revise one concept + examples","6:15–6:30 PM — Break","6:30–7:30 PM — Mathematics: focused problem practice","7:30–8:00 PM — Dinner / longer break","8:00–9:00 PM — Chemistry: concepts + practice questions","9:00–9:15 PM — Break","9:15–9:45 PM — Active recall + PYQs from today’s topics","9:45–10:00 PM — Review mistakes and plan tomorrow","","If you give me your subjects, exam dates, and available hours, I can make this more specific."].join("\n");
...
export function StudyAssistant() {
  ...
  const [messages, setMessages] = useState<Message[]>([{ role: "assistant", content: "Hi — I’m SkillNests AI. Ask me a question, give me a study task, or ask about SkillNests." }]);
  ...
  async function sendMessage(text = input) {
    ...
    try {
      if (shouldAnswerLocally(prompt, historyForRequest)) {
        ...
        if (answer) { setMessages((prev) => [...prev, { role: "assistant", content: answer }]); return; }
      }
      ...
      if (response.ok && data.answer) {
        ...
        setMessages((prev) => [...prev, { role: "assistant", content: data.error || localFallback(prompt, seconds, historyForRequest) || "I couldn't complete that request. Please try again." }]);
      }
    } catch (error) {
      const message = error instanceof DOMException && error.name === "AbortError" ? "The AI request timed out. Please try again." : "I couldn't connect to the AI service.";
  ...
  return (
    <>
      {open && (
        <div className="fixed bottom-24 right-4 z-[70] w-[min(420px,calc(100vw-2rem))] overflow-hidden rounded-3xl border border-rose-gold/20 bg-background/95 shadow-2xl backdrop-blur-xl">
          <div className="flex items-center justify-between border-b border-border/60 px-5 py-4">
            <div className="flex items-center gap-3"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-slate-950 text-cyan-300 shadow-lg ring-2 ring-cyan-300/30"><Bot className="h-7 w-7" strokeWidth={2.5} /></div><div><p className="font-serif text-xl">SkillNests AI</p><p className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Study • Explore • Focus</p></div></div>
      ...
      <div className="fixed bottom-5 right-5 z-[70] flex items-center gap-2"><div className="hidden items-center gap-1.5 rounded-full border border-border/70 bg-background/90 px-3 py-2 text-[11px] font-medium shadow-lg backdrop-blur-xl sm:flex"><Clock3 className="h-3.5 w-3.5 text-cyan-400" /><span className="text-muted-foreground">On site</span><span className="font-mono text-foreground">{duration}</span></div><button onClick={() => setOpen((value) => !value)} className="flex items-center gap-3 rounded-full border border-cyan-300/40 bg-slate-950 px-5 py-3.5 text-sm font-semibold text-white shadow-xl shadow-cyan-900/20 transition hover:-translate-y-0.5 hover:border-cyan-300/80 hover:bg-slate-900" aria-label="Open SkillNests AI" aria-expanded={open}><span className="flex h-8 w-8 items-center justify-center rounded-xl bg-cyan-300 text-slate-950 shadow-md"><Bot className="h-6 w-6" strokeWidth={2.5} /></span><span>SkillNests AI</span><Sparkles className="h-4 w-4 text-cyan-300" /></button></div>
    </>
  );
}
