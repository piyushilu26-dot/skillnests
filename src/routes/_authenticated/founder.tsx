        ...
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.65 }}>
          ...
          {(() => {
            ...
            const PersonCard = ({ f, role, featured = false }: { f: any; role: string; featured?: boolean }) => (
              <motion.article
                ...
                className={`group relative overflow-hidden rounded-[2rem] border border-rose-gold/10 bg-gradient-to-br from-background/90 via-background/70 to-rose-gold/[0.04] shadow-xl shadow-black/5 ${featured ? "p-7 sm:p-10" : "p-6 sm:p-7"}`}
              >
              ...
              <>
                <section>
                  <div className="mb-5 flex items-end justify-between gap-4">
                    <div>
                      <p className="text-[10px] font-mono uppercase tracking-[0.25em] text-rose-gold">01 / Leadership</p>
                      <h2 className="font-serif text-2xl sm:text-3xl mt-1">Founding Team</h2>
                    </div>
                  </div>
                  <div className="mb-8">
                    <PersonCard f={founders[0]} role={founders[0].role} featured />
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    {founders.slice(1).map((f) => <PersonCard key={f.email} f={f} role={f.role} />)}
                  </div>
                </section>
                ...
                <section className="mt-16">
                  <div className="mb-6 flex items-end justify-between gap-4">
                    <div>
                      <p className="text-[10px] font-mono uppercase tracking-[0.25em] text-rose-gold">02 / Core Team</p>
                      <h2 className="font-serif text-2xl sm:text-3xl mt-1">The Backbone</h2>
                    </div>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <form onSubmit={send} className="mt-16 rounded-[2rem] border border-rose-gold/10 bg-gradient-to-br from-background/80 to-rose-gold/[0.04] shadow-xl shadow-black/5 p-6 sm:p-8">
            <div className="flex items-center gap-3 mb-5">
              <div className="w-9 h-9 rounded-full border border-rose-gold/20 bg-rose-gold/5 flex items-center justify-center">
                <MessageCircle className="w-4 h-4 text-rose-gold" strokeWidth={1.2} />
              </div>
              <div>
                <h2 className="font-serif text-2xl">Start a conversation</h2>
                <p className="text-xs text-muted-foreground mt-0.5">Have an idea, question, or opportunity?</p>
              </div>
            </div>
