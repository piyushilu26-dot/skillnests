    ...
    <main className="min-h-screen">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 pt-28 pb-16">
        ...
        <div className="flex gap-2 mb-6 flex-wrap">
          <Tab active={tab === "careers"} onClick={() => setTab("careers")}>Explore careers</Tab>
...
function Tab({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <button onClick={onClick} className={`px-4 py-2 rounded-full text-sm transition ${active ? "glass-strong border-rose-gold/40 text-rose-gold" : "glass text-muted-foreground hover:text-foreground"}`}>
      {children}
    </button>
  );
}
  ...
  return (
    <div>
      <div className="glass-strong rounded-2xl p-5 mb-6">
        <div className="flex items-center gap-2 mb-3"><Compass className="w-5 h-5 text-rose-gold" /><h2 className="font-serif text-2xl">Explore career options</h2></div>
        <div className="flex gap-2 flex-wrap">{interests.map((i) => <button key={i} onClick={() => setInterest(i)} className={`px-3 py-1.5 rounded-full text-xs transition ${interest === i ? "btn-phoenix" : "glass text-muted-foreground hover:text-foreground"}`}>{i}</button>)}</div>
      </div>
      {isAdmin && <CareerGuideAdmin />}
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">{filtered.map((guide, index) => <CareerGuideCard key={guide.id} guide={guide} index={index} isAdmin={isAdmin} />)}</div>
...
function CareerGuideAdmin() {
  ...
  React.useEffect(() => {
    const handler = (event: Event) => { const guide = (event as CustomEvent).detail; if (!guide) return; setEditingId(guide.id); setDraft({ title: guide.title || "", interest: guide.interest || "", documentUrl: guide.documentUrl || "", isPaid: !!guide.isPaid }); window.scrollTo({ top: 0, behavior: "smooth" }); };
    window.addEventListener("skillnests:edit-career-guide", handler); return () => window.removeEventListener("skillnests:edit-career-guide", handler);
  }, []);
  const reset = () => { setEditingId(null); setDraft({ title: "", interest: "", documentUrl: "", isPaid: true }); };
  const submit = (e: React.FormEvent) => { e.preventDefault(); if (!draft.title || !draft.documentUrl) return; careerGuideStore.update((prev) => editingId ? prev.map((g) => g.id === editingId ? { ...g, ...draft, kind: "career-guide" as const, removed: false } : g) : [...prev, { id: uid(), kind: "career-guide" as const, speaker: "SkillNests Career Guide", ...draft }]); reset(); };
  return <form onSubmit={submit} className="glass-strong rounded-2xl p-4 mb-5 grid sm:grid-cols-2 gap-3">
    <div className="sm:col-span-2 flex items-center justify-between"><span className="text-xs uppercase tracking-widest font-mono text-rose-gold">{editingId ? "Edit career document" : "Add career document"}</span>{editingId && <button type="button" onClick={reset} className="text-xs text-muted-foreground hover:text-foreground flex items-center gap-1"><X className="w-3 h-3" /> Cancel</button>}</div>
...
function CareerGuideCard({ guide, index, isAdmin }: { guide: any; index: number; isAdmin: boolean }) {
  ...
  const open = () => { if (!guide.documentUrl) return; if (isDriveUrl(guide.documentUrl)) setViewing({ url: guide.documentUrl, title: guide.title }); else window.open(normalizeUrl(guide.documentUrl), "_blank", "noopener,noreferrer"); };
  return <>
    <div className="glass rounded-2xl overflow-hidden group relative">
      <div className="p-5">
        ...
        <div className="mt-4">{guide.isPaid ? <PaidGate label="₹49/month required"><button onClick={open} className="btn-ghost-gold rounded-full px-4 py-2 text-xs inline-flex items-center gap-1.5"><LockKeyhole className="w-3 h-3" /> Unlock document</button></PaidGate> : <button onClick={open} className="btn-phoenix rounded-full px-4 py-2 text-xs inline-flex items-center gap-1.5"><Eye className="w-3 h-3" /> View free document</button>}</div>
      </div>
      {isAdmin && <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition"><button onClick={() => window.dispatchEvent(new CustomEvent("skillnests:edit-career-guide", { detail: guide }))} title="Edit career document" className="glass-strong rounded-full p-1.5 text-muted-foreground hover:text-rose-gold"><Edit3 className="w-3.5 h-3.5" /></button><button onClick={() => careerGuideStore.update((p) => p.map((x) => x.id === guide.id ? { ...x, removed: true } : x))} title="Remove career document" className="glass-strong rounded-full p-1.5 text-muted-foreground hover:text-crimson"><Trash2 className="w-3.5 h-3.5" /></button></div>}
    </div>
...
function CareerResources({ isAdmin }: { isAdmin: boolean }) {
  ...
  const reset = () => { setEditingId(null); setDraft({ title: "", speaker: "", videoUrl: "", documentUrl: "", thumb: "" }); };
  const edit = (item: any) => { setEditingId(item.id); setDraft({ title: item.title || "", speaker: item.speaker || "", videoUrl: item.videoUrl || "", documentUrl: item.documentUrl || "", thumb: item.thumb || "" }); };
  const submit = (e: React.FormEvent) => { e.preventDefault(); if (!draft.title) return; careerVideoStore.update((prev) => editingId ? prev.map((x) => x.id === editingId ? { ...x, ...draft } : x) : [...prev, { id: uid(), ...draft }]); reset(); };
  return <div className="mt-10 pt-8 border-t border-white/5">
    <div className="mb-4"><div className="text-xs font-mono uppercase tracking-widest text-rose-gold">Additional career resources</div><h3 className="font-serif text-2xl">Guides, talks & documents</h3></div>
    {isAdmin && <form onSubmit={submit} className="glass-strong rounded-2xl p-4 mb-5 grid sm:grid-cols-2 gap-3">
      <div className="sm:col-span-2 flex items-center justify-between"><span className="text-xs uppercase tracking-widest font-mono text-rose-gold">{editingId ? "Edit career resource" : "Add career resource"}</span>{editingId && <button type="button" onClick={reset} className="text-xs text-muted-foreground flex items-center gap-1"><X className="w-3 h-3" /> Cancel</button>}</div>
    ...
    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      {items.map((item) => <div key={item.id} className="glass rounded-2xl overflow-hidden group relative">
        {item.videoUrl ? <div className="aspect-video relative"><div className="absolute inset-0 grid place-items-center"><Video className="w-10 h-10 text-rose-gold" /></div><a href={normalizeUrl(item.videoUrl)} target="_blank" rel="noreferrer" className="absolute inset-0" aria-label={`Open ${item.title}`} /></div> : <div className="aspect-video relative"><div className="absolute inset-0 grid place-items-center"><FileText className="w-10 h-10 text-rose-gold" /></div></div>}
        <div className="p-4"><div className="font-serif text-lg leading-tight">{item.title}</div>{item.speaker && <div className="text-xs text-muted-foreground mt-1">{item.speaker}</div>}{item.documentUrl && <div className="mt-3"><a href={normalizeUrl(item.documentUrl)} target="_blank" rel="noreferrer" className="btn-ghost-gold rounded-full px-4 py-2 text-xs inline-flex items-center gap-1.5"><FileText className="w-3 h-3" /> Open document</a></div>}</div>
