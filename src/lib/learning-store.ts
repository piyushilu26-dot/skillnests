...
export const learningEventsStore = createLocalStore<LearningEvent[]>(
  "sn-learning-events",
  [],
  (col, user) => query(col, where("userId", "==", user.uid))
);

export const learningProfilesStore = createLocalStore<LearningProfile[]>(
  "sn-learning-profiles",
  [],
  (col) => query(col)
);
...
export function recordLearningEvent(event: Omit<LearningEvent, "id" | "userId" | "createdAt">) {
  ...
  if (!user) return;
  learningEventsStore.update(items => [
    ...items,
    {
      ...event,
      id: uid(),
...
export function saveLearningProfile(input: Omit<LearningProfile, "id" | "userId" | "updatedAt">) {
  ...
  learningProfilesStore.update(items => existing
    ? items.map(p => p.userId === user.uid ? profile : p)
    : [...items, profile]
  );
}
